...
use honeycomb_cli_protocol::{CliAuthProbe, CliDriver, CliError, CliLaunchConfig, Result, PRESET_WORKER_BEE};
use honeycomb_cli_codex::CodexDriver;
...
impl WorkerBeeDriver {
    ...
    pub fn ensure_executable() -> Result<String> {
        ...
        Err(CliError::agent(format!(
            "找不到 worker-bee 可执行文件（解析为「{path}」）。请构建 worker-bee-cli，或设置 HONEYCOMB_WORKER_BEE_BIN"
        )))
    }
}
...
impl CliDriver for WorkerBeeDriver {
    ...
    async fn probe_auth(&self, cmd: &str, api_key_configured: bool) -> CliAuthProbe {
        ...
        CliAuthProbe {
            ...
            detail: if works { format!("worker-bee 可执行: {cmd}") } else { "请构建 worker-bee-cli 或设置 HONEYCOMB_WORKER_BEE_BIN".into() },
            api_key_configured,
        }
    }
}
