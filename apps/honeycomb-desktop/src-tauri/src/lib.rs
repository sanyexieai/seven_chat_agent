...
use honeycomb_server::build_app;
use tauri::Manager;
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,honeycomb_core=info,honeycomb_desktop=info".into()),
        )
        .init();
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let local_dir = resolver
                .app_local_data_dir()
                .ok()
                .unwrap_or_else(|| std::path::PathBuf::from(".honeycomb"));
            std::fs::create_dir_all(&local_dir).ok();
            let db_path = local_dir.join("honeycomb.db");
...
async fn boot_and_serve(
    ...
    let honeycomb = match Honeycomb::boot(&db_url).await {
        Ok(h) => h,
        Err(e) => {
