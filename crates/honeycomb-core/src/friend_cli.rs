...
pub use honeycomb_cli::{ClaudeDriver, CodexDriver, CursorDriver, WorkerBeeDriver};
...
pub fn is_external_cli_preset(cfg: &PtyBackendConfig) -> bool {
    honeycomb_cli::is_external_cli_preset(cfg.preset.as_deref())
}
...
pub async fn probe_external_cli_auth(
    ...
    honeycomb_cli::probe_auth(preset, cmd, api_key_configured).await
}
...
fn apply_external_cli_defaults(cfg: &mut PtyBackendConfig) {
    ...
    let cmd = match preset {
        PRESET_CODEX => "codex",
        PRESET_CURSOR => "agent",
        honeycomb_cli::PRESET_CLAUDE => "claude",
        _ => return,
    };
...
pub fn effective_pty_preset(cfg: &PtyBackendConfig) -> String {
    resolve_pty_preset(cfg).unwrap_or_else(|_| honeycomb_cli::PRESET_CLAUDE.into())
}
