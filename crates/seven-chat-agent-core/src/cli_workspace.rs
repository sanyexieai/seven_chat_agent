...
/// 工作区根目录：`SEVEN_CHAT_AGENT_CLI_WORKSPACE_ROOT`，否则 `{SEVEN_CHAT_AGENT_DATA}/cli-workspaces`。
pub fn workspace_root() -> PathBuf {
    if let Ok(root) = std::env::var("SEVEN_CHAT_AGENT_CLI_WORKSPACE_ROOT") {
        let t = root.trim();
        if !t.is_empty() {
            return PathBuf::from(t);
        }
    }
    let data = std::env::var("SEVEN_CHAT_AGENT_DATA").unwrap_or_else(|_| "data".into());
...
/// 默认 `true`；设 `SEVEN_CHAT_AGENT_CLI_AUTO_GIT=0` 可关闭自动 `git init`。
pub fn auto_git_enabled() -> bool {
    match std::env::var("SEVEN_CHAT_AGENT_CLI_AUTO_GIT")
        .unwrap_or_default()
        .to_lowercase()
        .as_str()
    {
...
/// 用户或 `SEVEN_CHAT_AGENT_CLI_CWD` 指定的目录：同样自动建目录并按需 init git。
