//! 蜂巢 CLI 驱动注册层：
//! - `seven-chat-agent-cli-protocol`：抽象接口与共享类型
//! - `seven-chat-agent-cli-codex` / `seven-chat-agent-cli-claude` / `seven-chat-agent-cli-cursor` / `seven-chat-agent-cli-worker-bee`：具体实现
pub mod registry;
pub mod resolve;

pub use seven_chat_agent_cli_protocol::{
...
pub use seven_chat_agent_cli_claude::ClaudeDriver;
pub use seven_chat_agent_cli_codex::CodexDriver;
pub use seven_chat_agent_cli_cursor::{CursorDriver, CURSOR_AGENT_ALIASES};
pub use seven_chat_agent_cli_worker_bee::WorkerBeeDriver;
pub use registry::{
