...
use honeycomb_judge::resolve_llm_target;
...
async fn check_judge_llm(
    ...
    let mut target = match resolve_llm_target(
        ...
        None => {
            errors.push(
                "任务流需要 Judge LLM：请在「群 Judge」中选择 Provider（或配置 HONEYCOMB_JUDGE_PROVIDER 环境变量）"
                    .into(),
            );
    ...
    if settings.judge.mode == honeycomb_judge::JudgeMode::Heuristic {
        warnings.push(
