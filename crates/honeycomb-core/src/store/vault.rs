...
///   `./data/vault.json`, overridable via `HONEYCOMB_VAULT`) with an
...
impl SecretVault {
    pub fn new() -> Self {
        let path = std::env::var("HONEYCOMB_VAULT")
            .map(PathBuf::from)
...
fn keychain_get(name: &str) -> Option<String> {
    let entry = keyring::Entry::new("honeycomb", name).ok()?;
    entry.get_password().ok()
}

#[cfg(feature = "keychain")]
fn keychain_set(name: &str, value: &str) -> std::result::Result<(), String> {
    let entry = keyring::Entry::new("honeycomb", name).map_err(|e| e.to_string())?;
