...
use seven_chat_agent_cli_protocol::{CliAuthProbe, CliDriver, CliError, CliLaunchConfig, CliSessionMode, Result, PRESET_CURSOR};
...
impl CursorDriver {
    ...
    pub fn resolve_executable(launch: &CliLaunchConfig) -> String {
        if let Ok(p) = std::env::var("SEVEN_CHAT_AGENT_CURSOR_AGENT_BIN") {
            let t = p.trim();
            if !t.is_empty() {
                return t.to_string();
            }
    ...
    pub fn ensure_executable(launch: &CliLaunchConfig) -> Result<String> {
        ...
        Err(CliError::agent("找不到 Cursor Agent CLI（尝试过 agent / cursor-agent）。请安装 Cursor Agent，或设置 SEVEN_CHAT_AGENT_CURSOR_AGENT_BIN"))
    }
}
