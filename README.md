...
3. **终端** — `crates/honeycomb-tui` 用 ratatui，通过 HTTP/WS 连上服务（本地或远端），可在 SSH 中跑。
...
| M7 | Tauri 2 桌面壳：嵌入 honeycomb-server 库，本地启动 `127.0.0.1:18739`，配置 Windows MSI / NSIS / macOS dmg / Linux AppImage / deb 打包 | ✓ |
| M8 | ratatui 终端壳：好友列表 + DM + WebSocket 流式 + judgment/scheduler 调试面板；`honeycomb-tui export/import` 配置导入导出 | ✓ |
...
cargo run --bin honeycomb-server
...
浏览器打开 <http://127.0.0.1:18738>。首启会创建 `data/honeycomb.db`，并预置 10 个 Provider 与内置助理 **Hex**。
...
1. 解析系统 app data 目录，把 SQLite 写到 `<app_data>/honeycomb.db`、把 vault 与 skills 文件放到同一目录；
...
如果改图标，把新 PNG/ICO/ICNS 放在 `apps/honeycomb-desktop/src-tauri/icons/` 下，文件名沿用 `tauri.conf.json` 中的 `bundle.icon` 列表。仓库自带的是占位图标（蜂巢轮廓加 h 字），打 Release 之前替换成正式品牌图标即可。

### 终端 UI

```bash
# 默认连本地 server
cargo run --bin honeycomb-tui

# 连远端 server
HONEYCOMB_SERVER=https://my-honeycomb.example/ cargo run --bin honeycomb-tui

# 导出 / 导入配置
cargo run --bin honeycomb-tui -- export --out cfg.json
cargo run --bin honeycomb-tui -- import cfg.json
```
...
3. 把 `https://<honeycomb-host>/?human=<code>` 发给真人朋友，对方打开后即可作为这位好友身份在群里发消息；正在输入时会触发"真人礼让"，AI 自动延迟出声。
...
```bash
cp .env.example .env
# 编辑 .env 后启动；honeycomb-server 会自动加载项目根目录的 .env
cargo run --bin honeycomb-server
```
...
| 变量 | 默认 | 含义 |
| --- | --- | --- |
| `HONEYCOMB_BIND` | `127.0.0.1:18737` | 后端监听地址 |
| `HONEYCOMB_DB`   | `sqlite://data/honeycomb.db` | SQLite 数据库 URL |
| `HONEYCOMB_DATA` | `data` | 数据目录 |
| `HONEYCOMB_VAULT` | `data/vault.json` | 本地凭据文件（启用 `keychain` feature 后会改走 OS keychain） |
| `HONEYCOMB_SERVER` | `http://127.0.0.1:18737` | TUI / 其他客户端连接的服务端 URL |
| `HONEYCOMB_STATIC_DIR` | 自动探测 `./web/dist` / `../web/dist` / `../../web/dist` | 让 `honeycomb-server` 直接托管前端静态资源；桌面壳里则用编译期内嵌（见下文） |
| `HONEYCOMB_CLI_CWD` | — | 所有 Pty 好友共用的工作目录（会建目录并默认 `git init`）；优先于自动工作区 |
| `HONEYCOMB_CLI_WORKSPACE_ROOT` | `{HONEYCOMB_DATA}/cli-workspaces` | 每位 Pty 好友自动工作区的父目录（子目录名为好友 `id`） |
| `HONEYCOMB_CLI_AUTO_GIT` | `1`（开启） | 新建/确保工作区时是否执行 `git init`；设 `0` 关闭 |
| `HONEYCOMB_SKILLS_DIR` | `data/skills` | 统一运行时技能库根目录 |

## 统一 Agent 运行时（`honeycomb-core/src/runtime`）
...
详见上文「统一 Agent 运行时」与仓库 `crates/honeycomb-core/src/runtime/`。
...
| `HONEYCOMB_ASSISTANT_PROVIDER` / `HONEYCOMB_ASSISTANT_MODEL` | — | Hex 助理在 backend_config 里没指定时的兜底 |

## 前端托管

`honeycomb-server` 既是 REST/WS 后端，也兼任前端的静态资源服务器：

1. **开发模式**：`npm run dev` 起 vite 自己代理 `/api`、`/ws`，server 不需要再托管前端。
2. **后端 + 已构建前端**：先在 `web/` 里 `npm run build`，然后直接跑 `cargo run --bin honeycomb-server`。server 会自动探测 `./web/dist`，浏览器打开 `http://127.0.0.1:18737/` 就能直接出页面，不必再起 vite。
3. **桌面壳 / 单文件部署**：`honeycomb-server` 的 `embed-frontend` feature 会在编译期把 `web/dist/` 通过 `include_dir!()` 烧进二进制；桌面壳（`apps/honeycomb-desktop`）默认就启用这个 feature，因此 `cargo tauri build` 出来的安装包不依赖运行时存在 `web/dist`。

如要在自定义部署里嵌入前端：

```bash
cd web && npm install --no-bin-links && npm run build && cd ..
cargo build --release -p honeycomb-server --features embed-frontend
```
...
│   ├── honeycomb-server/           axum HTTP + WS（同时是 lib，桌面壳复用）
│   ├── honeycomb-tui/              ratatui 终端壳
│   └── worker-bee-cli/             工蜂 Worker Bee CLI（`worker-bee` bin，memory/MCP/skill）
├── apps/
│   └── honeycomb-desktop/          Tauri 2 桌面壳
...
`honeycomb-server` 暴露的 REST + WS 端点：
